...
bool MapContents::__readAirspaceFile(const QString& pathName)
{
  // qDebug ("reading file %s", pathName.toLatin1().data() );
  ...
  // Got to initialize "out" stream properly, even if write file is not
  // needed
  ...
  qint16 uLimit, lLimit;
  BaseMapElement::elevationType uLimitType, lLimitType;
  ...
  //  Q3PtrList<radioContact> contactList;
  ...
  if(!mapfile.open(IO_ReadOnly)) {
    // Can't read input data:
    // We need to output a warning...
    qWarning("Cumulus: Can not open airspace file %s!",
             pathName.toLatin1().data());
    ...
    if( ! compiling && kflExists ) {
      ...
      // @AP: make a second try with the source file. Can fail, if
      // source was removed but we are optimists
      return __readAirspaceFile(kflPathName);
    }
  ...
  //qDebug ("open airspacefile: %s", pathName.toLatin1().data());
  ...
  if (compiling) {
    ...
    if(formatID < FILE_VERSION_AIRSPACE) {
      // to old ...
      qWarning("Cumulus: File format too old! (version %d, expecting: %d) "
               "Aborting ...",
               formatID, FILE_VERSION_AIRSPACE);
      mapfile.close();
      return false;
    } else if(formatID > FILE_VERSION_AIRSPACE) {
      // to new ...
      qWarning("Cumulus: File format too new! (version %d, expecting: %d.). "
               "Aborting ...",
               formatID, FILE_VERSION_AIRSPACE);
  ...
  } else  // compiled version is checked here
    {
      if(loadTypeID != FILE_TYPE_AIRSPACE_C) {
        qWarning("Cumulus: wrong load type identifier %x read!", loadTypeID );
        mapfile.close();
      ...
      if(formatID < FILE_VERSION_AIRSPACE_C) {
        // to old ...
        qWarning("Cumulus: File format too old! (version %d, expecting: %d.)",
                 formatID, FILE_VERSION_AIRSPACE_C);
        mapfile.close();
      ...
      } else if(formatID > FILE_VERSION_AIRSPACE_C) {
        // to new ...
        qWarning("Cumulus: File format too new! (version %d, expecting: %d.)"
                 "Retry to compile kfl.",
                 formatID, FILE_VERSION_AIRSPACE_C);
        mapfile.close();
  ...
  if( ! compiling ) {
    ...
    // because we're using a buffer, the length will be written too. This
    // has the same type as lat_temp, and it is not used.
    in >> lat_temp;
  }
  ...
  if ( compiling ) {
    ...
    //create and prepare our buffer and the stream to it...
    buffer.open(IO_ReadWrite);
    outbuf.setDevice(&buffer);
    outbuf.setVersion(2);
  }
  ...
  while(!in.atEnd()) {
    ...
    case BaseMapElement::Tmz:
      ...
      if ( compiling ) {
        ...
        in >> idString;
        in >> icaoName;
        in >> (quint8&)lLimitType;
        in >> lLimit;
        in >> (quint8&)uLimitType;
        in >> uLimit;
        in >> exception;
        //qDebug("name: %s, icaoName: %s, id: %s", name.toLatin1().data(), icaoName.toLatin1().data(), idString.toLatin1().data());
        ShortSave(outbuf, name);
        //ShortSave(out, idString);    -> not used now
        //ShortSave(out, icaoName); -> not used now
        outbuf << (quint8&)lLimitType;
        outbuf << lLimit;
        outbuf << (quint8&)uLimitType;
        outbuf << uLimit;
        outbuf << exception;
      }  else {
        ShortLoad(in, name);
        //ShortLoad(in, idString);  -> not used now
        //ShortLoad(in, icaoName);-> not used now
        in >> (quint8&)lLimitType;
        in >> lLimit;
        in >> (quint8&)uLimitType;
        in >> uLimit;
      ...
      if (exception != 0) {
        if ( compiling ) {
          in >> exceptionText;
          ShortSave(outbuf, exceptionText);
        } else {
          ShortLoad(in, exceptionText);
        }
      }
      ...
      for(unsigned int loop = 0; loop < contactCount; loop++) {
        if ( compiling ) {
          ...
          in >> contactType;
          in >> contactCallSign;
          ShortSave(outbuf, contactFrequency);
          outbuf << contactType;
          ShortSave(outbuf, contactCallSign);
        }
        {
          ShortLoad(in, contactFrequency);
          in >> contactType;
          ShortLoad(in, contactCallSign);
        }
      }

      //              READ_POINT_LIST
      //this is READ_POINT_LIST slightly modified.
      //this will go back to a macro once the other pointlists are stored in the same way.

      if (compiling) {
        in >> locLength;
        // qDebug("number of points in structure %d",locLength);
        pN.resize(locLength);
        for(unsigned int i = 0; i < locLength; i++) {
          in >> lat_temp;
          in >> lon_temp;
      ...
      airspaceList.append(new Airspace(name, typeIn, pN,
                                       uLimit, uLimitType, lLimit, lLimitType));
      //set bounding rectangle to include this pointarray
      if (!pN.isEmpty())
        boundingBox |= pN.boundingRect();


      break;
    default:
      qWarning ("MapContents::__readAirspaceFile; type not handled in switch: %d", typeIn);
    }
  ...
  if ( compiling ) {
    ...
    //write data on airspaces from bufferdata
    out << bufdata;
  ...
  ws->slot_SetText2(tr("Loading map ready"));
  qDebug("Read AirSpaceFile %s in %dms", pathName.toLatin1().data(), t.elapsed());
  return true;
}
...
#if 0 // KFLog airspace files not more supported by cumulus X11, AP 02.02.2008


      addDir(preselect, mapDir1 + "/airspace", "*.kfc");
      addDir(preselect, mapDir1 + "/airspace", "*.kfl");
      addDir(preselect, mapDir2 + "/airspace", "*.kfc");
      addDir(preselect, mapDir2 + "/airspace", "*.kfl");
      addDir(preselect, mapDir3 + "/airspace", "*.kfc");
      addDir(preselect, mapDir3 + "/airspace", "*.kfl");

      // Now, we have to test for updated or uncompiled kfls ... Anyone can do it more elegantly?
      ...
      if(preselect.count() == 0) {
        ...
        while ( ! preselect.isEmpty() ) {
          if ( preselect.first().contains( QString(".kfl") ) ) {
            ...
            airspace.append( preselect.first() );
            ...
            if ( kflPathName == preselect.first() ) {
              if ( getDateFromMapFile(kflPathName) >
                ...
                airspace.append( kflPathName );
                qDebug("Airspace file %s has a newer date, recompiling",
                       kflPathName.toLatin1().data() );
              } else {
                airspace.append( kfcPathName );
