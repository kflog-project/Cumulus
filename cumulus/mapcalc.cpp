 ...
 */
double MapCalc::distP(double lat1, double lon1, double lat2, double lon2)
{
 ...
 */
double MapCalc::distC1(double lat1, double lon1, double lat2, double lon2)
{
...
QPair<double, double> MapCalc::distVinc( QPoint *p1, QPoint *p2 )
{
  return ( distVinc( double(p1->x()), double(p1->y()),
                     double(p2->x()), double(p2->y()) ));
}

double MapCalc::dist( QPoint *p1, QPoint *p2 )
{
...
double MapCalc::getBearing(QPoint p1, QPoint p2)
{
...
double MapCalc::getBearing2(QPoint p1, QPoint p2)
{
...
*/
double MapCalc::getBearingWgs( QPoint p1, QPoint p2 )
{
...
double MapCalc::polar(double y, double x)
{
  double angle = 0.0;
  //
  //  dX = 0 ???
 ...
 */
double MapCalc::outsideVector(QPoint center, QPoint p1, QPoint p2)
{
...
QPoint MapCalc::getPosition( QPoint startPos, double distance, int direction )
{
  if( distance == 0 )
    {
      return QPoint();
 ...
 */
QRect MapCalc::areaBox(QPoint center, double r)
{
  const double pi_180 = M_PI / 108000000.0;
 ...
 */
QRect MapCalc::getTileBox(const ushort tileNo)
{
  if( tileNo > (180*90) )
    {
 ...
 */
int MapCalc::mapTileNumber( double lat, double lon )
{
 ...
 */
bool MapCalc::windTriangle( const double trueCourse,
		        ...
		        int& eth )
{
  ...
  qDebug() << "tk=" << tk
           ...
           << "wd=" << wd
           << "ws=" << ws.getKph()
           << "tas" << tasv.getSpeed().getKph()
           << "th=" << tasv.getAngleDeg();
  */
  etas = tasv.getSpeed();
  eth  = tasv.getAngleDeg();
