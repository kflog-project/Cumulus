...
public class BluetoothService
{
  ...
  /**
   * Constructor. Prepares a new BluetoothService session. The session can be
   * used as server or client.
   * 
   * @param context
  ...
  /**
   * Setup a Bluetooth server socket for incoming connections. Specifically
  ...
  /**
   * Indicates that the connection attempt to the remote device failed and
   * notifies the UI Activity.
   */
  ...
  private void connectionLost()
  {
    ...
    Bundle bundle = new Bundle();
    bundle.putString(BluetoothService.TOAST,
        mContext.getString(R.string.btError2));
    msg.setData(bundle);
    mHandler.sendMessage(msg);
   ...
   * a server-side client. It runs until a connection is accepted (or until
   * cancelled).
   */
      ...
      while (mState != STATE_CONNECTED)
        {
          ...
          if (socket != null)
            {
              Log.v(TAG, "accept(): Device="
                  + socket.getRemoteDevice().getName() + "Mac="
  ...
  private class ConnectThread extends Thread
  {
     ...
     * 
     * @param device
     *          The remote device to be connected.
     */
    ...
    public void run()
    {
      ...
      try
      ...
      catch (IOException e)
        {
          Log.e(
              TAG,
              "ConnectThread: Unable to connect() to device='"
  ...
  private class ConnectedThread extends Thread
  {
              ...
              if (character == -1)
                {
                  ...
                  if (getAbort() == false)
                    {
                      // Only notify a connection lost, if the socket was not
                      // closed
                      // by our software.
                      connectionLost();
                    }
    ...
    {
      if (D)
        Log.d(TAG, "ConnectedThread: cancel " + this);
