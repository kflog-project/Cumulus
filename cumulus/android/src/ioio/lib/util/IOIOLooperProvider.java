...
public interface IOIOLooperProvider
{
   ...
   * returned if the client is not interested to create a thread for this IOIO.
   * This can be used in order to filter out unwanted connections, for example
   * if the application is only intended for wireless connection, any wired
   * connection attempts may be rejected, thus saving resources used for
