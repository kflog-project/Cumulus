...
{
  /**
   * Subclasses should override this method for performing operations to be done
   * once as soon as IOIO communication is established.
   */
  ...
  /**
   * Subclasses should override this method for performing operations to be done
   * repetitively as long as IOIO communication persists. Typically, this will
   * be the main logic of the application, processing inputs and producing
   * outputs.
   */
  ...
  /**
   * Subclasses should override this method for performing operations to be done
   * once as soon as IOIO communication is lost or closed. Typically, this will
   * include GUI changes corresponding to the change. This method will only be
   * called if setup() has been called. The ioio argument passed to
   * {@link #setup(IOIO)} must not be used from within this method - it is
   * invalid. This method should not block for long, since it may cause an ANR.
   */
  public abstract void disconnected();

  /**
   * Subclasses should override this method for performing operations to be done
   * if an incompatible IOIO firmware is detected. The ioio argument passed to
   * {@link #setup(IOIO)} must not be used from within this method - it is
   * invalid. This method will only be called once, until a compatible IOIO is
   * connected (i.e. {@link #setup(IOIO)} gets called).
   */
