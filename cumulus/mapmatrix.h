...
public:
    ...
    */
    ...
    /** This function tries to make the given point visible on the map, using only scaling.
      * This may fail if the point is too far away, so that the requiered scale is bigger
      * than the limit. If the function fails, the current projection is not changed and
      * false is returned. If the function succeeds, the scale is changed so that the greatest
      * level of detail can be seen while keeping the point on the map. The new scale is returned. */
...
private:
    ...
    // QRect printBorder;
    QRect mapBorder;
    //the mapCenterArea is the rectangle in which the glidersymbol can move around before the map is redrawn
    QRect mapCenterArea;
